...
class Controller():
    ...
    def Func_Criar_Treeview(self, verificar=True):
        ...
        if verificar == True:
            ...
            self.frame_lista = self.verificador_treeview(value='frame')
            self.Lista_Treeview = ttk.Treeview(
    ...
    def Atualiza_TreeView(self, frame='.!frame3.!treeview', verificar=True):
        # Atualiza a lista
        frame = frame
        self.Lista_Treeview = self.root.nametowidget(frame)
        self.Lista_Treeview.delete(*self.Lista_Treeview.get_children())
        for row in self.banco.Func_Select_Lista(verificar):
            if verificar:
    ...
    def Atualiza_db(self, value, verificar):
        ...
        if dados_v == True:
            ...
            self.Func_Erro_Dados(self.root, frase='Cod Produto ou cliente não Cadastrado')
